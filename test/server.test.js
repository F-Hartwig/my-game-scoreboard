...
test('public access is limited to explicit read-only preview', async t => {
    ...
    assert.equal((await anonymous.request('/personal-stats.mjs')).response.status, 200);
    assert.equal((await anonymous.request('/setup-player-order.mjs')).response.status, 200);
    assert.equal((await anonymous.request('/werwolf-day-resolution.mjs')).response.status, 200);
...
test('Werwolf games are unranked, validate secret state, and never expose secrets in preview', async t => {
    ...
    const game = { id: 990, gameTypeId: 'werwolf', name: 'Werwolf', mode: 'assistant', rated: false,
        ...
        werewolf: { version: 1, phase: 'night', number: 1, step: 'amor', revealOnDeath: false,
            roles: [{ playerId: 101, roleId: 'gamemaster', baseTeam: 'moderator', currentTeam: 'moderator', alive: false }, { playerId: 102, roleId: 'werewolf', baseTeam: 'wolves', currentTeam: 'wolves', alive: true }, { playerId: 103, roleId: 'seer', baseTeam: 'village', currentTeam: 'village', alive: true }], events: [] } };
...
test('Werwolf validation requires one game master and refuses duplicate unique roles', async t => {
    const f = await fixture(t);
    const admin = await setupAdmin(f);
    await savePlayers(admin);
    const game = { id: 991, gameTypeId: 'werwolf', name: 'Werwolf', mode: 'assistant', rated: false,
        players: [{ id: 101, name: 'Alice', playerIds: [101], rounds: [], total: 0 }, { id: 102, name: 'Bob', playerIds: [102], rounds: [], total: 0 }, { id: 103, name: 'Cara', playerIds: [103], rounds: [], total: 0 }],
        werewolf: { version: 1, phase: 'night', number: 1, step: 'amor', revealOnDeath: false,
            roles: [{ playerId: 101, roleId: 'gamemaster', baseTeam: 'moderator', currentTeam: 'moderator', alive: false }, { playerId: 102, roleId: 'werewolf', baseTeam: 'wolves', currentTeam: 'wolves', alive: true }, { playerId: 103, roleId: 'seer', baseTeam: 'village', currentTeam: 'village', alive: true }], events: [] } };
    assert.equal((await admin.post('/api/active-games', game)).response.status, 201);
    const missingGameMaster = structuredClone(game);
    missingGameMaster.id = 992;
    missingGameMaster.werewolf.roles[0] = { playerId: 101, roleId: 'villager', baseTeam: 'village', currentTeam: 'village', alive: true };
    assert.equal((await admin.post('/api/active-games', missingGameMaster)).response.status, 400);
    const duplicateSeer = structuredClone(game);
    duplicateSeer.id = 993;
    duplicateSeer.werewolf.roles[1] = { playerId: 102, roleId: 'seer', baseTeam: 'village', currentTeam: 'village', alive: true };
    assert.equal((await admin.post('/api/active-games', duplicateSeer)).response.status, 400);
});
...
test('guests are created atomically for one game and become regular players only when a master promotes them', async t => {
    const f = await fixture(t);
    const admin = await setupAdmin(f);
    await savePlayers(admin);
    assert.equal((await admin.post('/api/users', { username: 'alice', password: USER_PASSWORD, playerId: 101 })).response.status, 201);
    ...
    const temporaryGuestId = -901;
    const game = { id: 901, name: 'Gastpartie', mode: 'round', rated: true, guestDrafts: [{ id: temporaryGuestId, name: 'Gast Gabi' }], players: [
        { id: 101, name: 'Alice', playerIds: [101], rounds: [3], total: 3 },
        { id: temporaryGuestId, name: 'Gast Gabi', playerIds: [temporaryGuestId], rounds: [7], total: 7 }
    ], winner: 'Gast Gabi', winnerPartyIds: [temporaryGuestId] };
    const created = await user.post('/api/active-games', game);
    assert.equal(created.response.status, 201, created.text);
    const guest = created.data.players.find(player => player.name === 'Gast Gabi');
    assert.ok(Number.isSafeInteger(guest.id));
    assert.ok(guest.id >= 0);
    assert.equal(created.data.guestDrafts, undefined, 'temporary guest details are not persisted in the game');
    assert.equal((await user.request(`/api/guests?gameId=${game.id}`)).response.status, 403);

    const finished = await user.post('/api/active-games/901/finish', created.data);
    assert.equal(finished.response.status, 200, finished.text);
    assert.equal(finished.data.players.some(player => player.id === guest.id), false, 'guest is not a regular player');
    assert.equal((await user.put(`/api/favorites/${guest.id}`, { favorite: true })).response.status, 404);
    assert.equal((await user.post(`/api/guests/${guest.id}/promote`, {})).response.status, 403);
    assert.equal((await admin.request(`/api/guests?gameId=${game.id}`)).data.some(entry => entry.id === guest.id), true);
    assert.deepEqual((await admin.request('/api/guests?gameId=999')).data, [], 'a guest is not visible for other games');

    const promoted = await admin.post(`/api/guests/${guest.id}/promote`, {});
    assert.equal(promoted.response.status, 200, promoted.text);
    assert.equal(promoted.data.id, guest.id);
    ...
    assert.deepEqual((await admin.request(`/api/guests?gameId=${game.id}`)).data, []);
    const players = (await admin.request('/api/players')).data;
    assert.deepEqual(players.find(player => player.id === guest.id), promoted.data, 'promoted ID preserves historical references');
    assert.equal((await admin.post(`/api/guests/${guest.id}/promote`, {})).response.status, 404);
});
...
test('each account manages an independent player favorite list', async t => {
    ...
    assert.equal((await admin.put('/api/favorites/101', { favorite: true })).response.status, 200);
    assert.equal((await alice.put('/api/favorites/102', { favorite: true })).response.status, 200);
    const adminFavorites = await admin.request('/api/favorites');
    assert.deepEqual(adminFavorites.data, ['101']);
    assert.equal(adminFavorites.response.headers.get('cache-control'), 'no-store');
    assert.deepEqual((await alice.request('/api/favorites')).data, ['102']);
    assert.equal((await alice.put('/api/favorites/999', { favorite: true })).response.status, 404);
    assert.equal((await alice.put('/api/favorites/102', { favorite: false }, { csrf: false })).response.status, 403);
    assert.equal((await anonymous.request('/api/favorites')).response.status, 401);
    assert.equal((await anonymous.put('/api/favorites/101', { favorite: true })).response.status, 401);
